...
class Solution:
    def addTwoNumbers(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
            ...
            head_node = ListNode( int(str(ans)[-1]) )
            
            tail_node = head_node  # head node is also seen as last node
            
            for x in list(str(ans)[::-1])[1:]: #convert ans to str; reverse it; loop from index 1
                # each iteration ref new created node, since the new created node become tail node
                tail_node = insertNode(tail_node, ListNode(int(x))) 
